...
/**
 * <p>
 * <code>WebSearchHitIterator</code> Iterator for search results which contain WebSearchHit documents.<br>
...
public final class WebSearchHitIterator {
	...
	/**
	 * Insert the method's description here. Creation date: (2/16/2001 2:06:31 PM)
	 */
	 ...
	 * WebSearchHit hit = null;
	 * 
	 * if (hasNext()) { try { position++; Document doc = hits.doc(position - 1); WebSearchURL url = database.get(doc.get("keyURL")); hit = new
	 * WebSearchHit(url, doc, position, hits.score(position - 1)); } catch (Exception e) { e.printStackTrace(); } }
	 * 
	 * return hit;
	 */
	public void nextSet() {

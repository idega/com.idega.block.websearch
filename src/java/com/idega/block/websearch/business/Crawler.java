...
import org.apache.lucene.analysis.StopAnalyzer; // import org.apache.lucene.document.DateField;
...
public final class Crawler {
	...
	public void crawl() {
		try {
			...
			/*
			 * else { // delete all files for now and build new index. // implement incremental index later. if (reporting > 0) System.out.println("index
			 * exists, delete all files"); //FileUtil.delete(indexPath);
			 * 
			 * //delete all IndexReader reader = IndexReader.open(indexPath); int count = reader.numDocs(); if (reporting > 0) {
			 * System.out.println("deleting " + count + " records"); } for (int i = 0; i < count; i++) { //if (reporting > 1) System.out.println("deleted " +
			 * i); reader.delete(i); } reader.close();
			 * 
			 * System.out.println("Websearch: creating index folders..."); System.out.println("Websearch: "+indexPath);
			...
			while (this.linkQueue != null && !this.linkQueue.empty()) {
				...
				if (result.equals("good")) {
					...
					if (this.reporting > 2) {
						...
						System.out.println(" robot rules: index=" + this.handler.getRobotIndex() + "  follow=" + this.handler.getRobotFollow());
	...
	/**
	 * Takes an url and rearranges it so all query parameters are in the same order as another url with the same query parameters. This helps eliminate
	 * duplicates. It also changes servlet/IBMainServlet/? and servlet/IBMainServlet? to index.jsp?
	 * 
	 * @param url
	 * @return
	 */
	...
	/*
	 * Index the html in lucene. Path is the url path, and contents is the parsed html
	 */
	private void indexLucene() {
		try {
			...
			if (this.handler.getHREF() != null) {
				...
				href = href.substring(0, pos) + this.currentURL.toString() + href.substring(pos + 5, href.length());
